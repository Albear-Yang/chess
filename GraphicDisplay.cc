...
void GraphicsDisplay::notify()
{
    ...
    if (subj->round1 || subj->undoCalled || subj->pastMoves.empty())
    {
        for (int i = 0; i < boardLen; i++)
        {
            ...
            {
                if ((i + j) % 2 == 0)
                    w->fillRectangle(50 * (j + 2), 50 * i, 50, 50, Xwindow::White);
                else
                    w->fillRectangle(50 * (j + 2), 50 * i, 50, 50, Xwindow::Green);
                ;
